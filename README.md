...
AI エージェントに Full Access 相当の権限を与えて自動作業させる場合は、Docker、仮想マシン、AI エージェント専用の物理マシンなど、壊れても良い隔離環境での利用を推奨します。

Docker を利用する場合のサンプル設定は [docker/README.md](./docker/README.md) を参照してください。

利用ワークフローについては [docs/Workflow.md](./docs/Workflow.md) を参照してください。
...
アンインストールする場合は、このリポジトリルートで以下を実行します。
...
- `uninstall.sh` は、インストール先の各対象ディレクトリ直下を走査し、このリポジトリ配下を指しているシンボリックリンクだけを削除します。
  - 通常のファイルやディレクトリは削除しません。
  - 他の場所を指しているシンボリックリンクは削除しません。
  - 削除後に空になった対象ディレクトリや親ディレクトリは、空の場合に限って削除します。
